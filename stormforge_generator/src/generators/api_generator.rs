use crate::ir::{IRModel, Command, Query};
...
pub struct ApiResponseString {{
    ...
    fn generate_router(model: &IRModel) -> Result<String> {
        ...
        for (name, _) in &model.commands {
            ...
            code.push_str(&format!("        .route(\"{}\", post({}))\n", endpoint, handler));
        }
        ...
        for (name, _) in &model.queries {
            let endpoint = format!("/{}", to_kebab_case(name));
            let handler = to_snake_case(name);
            code.push_str(&format!("        .route(\"{}\", get({}))\n", endpoint, handler));
        }
    ...
    fn generate_command_handler(name: &str, command: &Command) -> Result<String> {
        ...
        code.push_str(&format!("        (status = 200, description = \"Command executed successfully\"),\n"));
        code.push_str(&format!("        (status = 400, description = \"Invalid request\", body = ApiError),\n"));
        code.push_str(&format!("        (status = 500, description = \"Internal server error\", body = ApiError)\n"));
        code.push_str(&format!("    )\n"));
        code.push_str(&format!(")]\n"));
    ...
    fn generate_query_handler(name: &str, query: &Query) -> Result<String> {
        ...
        code.push_str(&format!("    path = \"/{}\",\n", to_kebab_case(name)));
        code.push_str(&format!("    responses(\n"));
        code.push_str(&format!("        (status = 200, description = \"Query executed successfully\"),\n"));
        code.push_str(&format!("        (status = 404, description = \"Not found\", body = ApiError),\n"));
        code.push_str(&format!("        (status = 500, description = \"Internal server error\", body = ApiError)\n"));
        code.push_str(&format!("    )\n"));
        code.push_str(&format!(")]\n"));
        
        code.push_str(&format!("pub async fn {}() -> impl IntoResponse {{\n", handler_name));
